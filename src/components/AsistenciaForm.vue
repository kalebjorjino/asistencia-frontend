        ...
        <div class="card-body p-4">
          ...
          <form @submit.prevent="emit('submit', formData)">
            <div class="mb-3">
              ...
              <input type="number" id="dni" v-model="formData.dni" class="form-control" placeholder="Ingrese su DNI"
                required />
            </div>

            <div class="mb-3">
              <label for="ubicacion" class="form-label fw-bold">Ubicación:</label>
              <input type="text" id="ubicacion" v-model="formData.ubicacion" class="form-control" disabled required />
            </div>

            <div class="mb-3">
              <button type="button" class="btn btn-secondary form-control" @click="triggerCamera">
                Abrir Cámara
              </button>
              <input ref="inputFile" type="file" id="foto" accept="image/*" capture="camera" @change="handleFileUpload"
                class="d-none" required />
              <div v-if="formData.foto" class="mt-2 d-flex justify-content-center">
                <img :src="previewImage" alt="Foto de Asistencia" class="img-fluid rounded"
                  style="max-height: 150px;" />
              </div>
            </div>

            <button type="submit" class="btn btn-primary btn-block form-control" :disabled="!formData.foto">
              Asistencia
            </button>
          </form>
